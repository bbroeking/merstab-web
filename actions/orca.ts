...
export const swap = async (
    ...
    const swapPayload: TransactionPayload = await whETHUSDCPool.swap(owner, whETH, whETHAmount, usdcAmount);
    const tx = await wallet.sendTransaction(swapPayload.transaction, connection, { signers: swapPayload.signers, skipPreflight: true });
